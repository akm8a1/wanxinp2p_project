package com.liu.wanxinp2p.repayment.util;

import com.liu.wanxinp2p.repayment.model.EqualInterestRepayment;
import com.liu.wanxinp2p.repayment.model.EqualPrincipalRepayment;
...
public class RepaymentUtil {
    ...
    private static Map<Integer, BigDecimal> getRepaymentPrincipalList(BigDecimal invest, Double monthRate, int month) {
        ...
        if (monthRate <= 0) {
            monthIncome = invest.divide(new BigDecimal(month),BigDecimal.ROUND_DOWN);
        } else {
            monthIncome = invest.multiply(new BigDecimal(monthRate * Math.pow(1 + monthRate, month)))
                    .divide(new BigDecimal(Math.pow(1 + monthRate, month) - 1), 2, BigDecimal.ROUND_DOWN);
        }
    ...
    private static BigDecimal getRepaymentTotalAmount(BigDecimal invest, double monthRate, int month) {
        BigDecimal perMonthInterest = null;
        if (monthRate <= 0) {
            perMonthInterest = invest.divide(new BigDecimal(month),BigDecimal.ROUND_DOWN);
        } else {
            perMonthInterest = invest.multiply(new BigDecimal(monthRate * Math.pow(1 + monthRate, month)))
                    .divide(new BigDecimal(Math.pow(1 + monthRate, month) - 1), 2, BigDecimal.ROUND_DOWN);
        }
